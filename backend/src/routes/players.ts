    ...
    if (parentContributions.car) {
      ...
      if (vehicleLookup) {
        const vehicle = await prisma.vehicle.findUnique({
          where: {
            name_ageVariant: {
              name: vehicleLookup.name,
              ageVariant: vehicleLookup.ageVariant,
            },
          },
        });
        if (vehicle) {
          vehicleOwnershipData = { vehicleId: vehicle.id };
        }
      }
    }
    ...
    const updatedPlayer = await prisma.$transaction(async (tx) => {
      ...
      if (vehicleOwnershipData) {
        await tx.vehicleOwnership.create({
          data: {
            playerId: id,
            vehicleId: vehicleOwnershipData.vehicleId,
            startAge: updated.age,
            wasParentGift: true,
          },
        });
      }
